...
export const coreSchema = {
  ...
  courses: defineCampusSchema('courses', {
    ...
    fields: {
      ...
      name: { default: '', type: 'text', label: '課程名稱', required: true },
      description: { default: '', type: 'textarea', label: '描述', showInTable: false },
      maxStudents: { default: 10, type: 'number', label: '人數上限', showInTable: false, span: 1, min: 0 },
      teacherId: { default: '', type: 'select', label: '授課教師', optionsKey: 'teachers', span: 1 },
      billingType: {
      ...
      unitPrice: {
        default: 0, type: 'number', label: '單價', showInTable: false, span: 1,
        showIf: (model) => model.billingType === 'weekly-by-lesson'
      },
      fixedTotalAmount: {
        default: 0, type: 'number', label: '總價', showInTable: false, span: 1,
        showIf: (model) =>
          model.billingType === 'weekly-total' || model.billingType === 'period-total'
      },
      schedules: {
        default: [], type: 'custom', label: '上課時間', component: 'TimePeriodArray',
        format: (v) => formatTimePeriodArray(v, 'range'),
        required: true, showInForm: false
      },
      period: {
        default: {}, type: 'custom', label: '期間', component: 'DatePeriod',
        format: (v) => formatDatePeriod(v, 'range'), showInTable: false, span: 2
      },
      isValid: { default: true, type: 'checkbox', label: '啟用', showInForm: false, showInTable: false }
    },
    searchFields: ['name', 'teacherId', 'campusId'],
    validate: (form) => {
      const errors = {}

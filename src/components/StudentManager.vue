...
const refreshStudents = async () => {
  try {
    ...
    const [studentsData, coursesData, settingsData] = await Promise.all([
      studentService.getStudents(), // 🚀 同時出發
      courseService.getAllCoursesWithDeleted(),
      adminService.getSettings(),
    ]);
    localStudents.value = studentsData;
    systemCourses.value = coursesData;
...
// 先確保 student 存在，再檢查 courseIds
const getValidCourseCount = (student) => {
  // 如果 student 不存在，直接回傳 0，徹底解決 Cannot read properties of undefined
  if (!student) return 0;

  // 如果沒有 courseIds，也回傳 0
  if (!student.courseIds || !Array.isArray(student.courseIds)) return 0;

  // 過濾掉無效課程（isValid 為 false 的課程）
  const validCourses = student.courseIds.filter((id) => {
    const course = systemCourses.value.find((c) => c.id === id);
    return course && course.isValid !== false;
  });
...
<template>
  <div class="student-manager">
    ...
    <StudentCourseModal
      ...
      :allCourses="systemCourses"
      @saved="refreshStudents"
    />
    <GenerateBillModal
